...
if (isset($_POST['usrloginbtn'])) {
   ...
   $query = $DBcon->query("select * from  cusers where (user = '$email' OR email = '$email')");
   ...
   if (password_verify($password, $row['pass']) && $count==1) {
    $_SESSION['user'] = $row['cuser_id'];
    setcookie("user", $_SESSION['user'], time() + (86400 * 30 * 12), "/");
    header("Location: userarea.php");
   } else {
	...
	<div class="row">
		<div class="col-md-6 col-md-offset-3 col-sm-12 col-xs-12  ">
          ...
          <center><!--div type="submit" name="usrloginntn" class="sim-button button8"><span>Login</span></div-->
            <button type="submit" class="btn-block btn sub-btn" name="usrloginbtn">Login</button>
          </center>
      <br>
